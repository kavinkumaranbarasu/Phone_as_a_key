...
public class MainViewModel : AndroidViewModel {
    ...
    //region NFC Methods
    public fun onCheckNFC(isChecked : Boolean) { Coroutines.io(this@MainViewModel) {
    ...
    public fun readTag(tag : Tag?) { Coroutines.default(this@MainViewModel) {
    ...
    private suspend fun postNFCStatus(status : NFCStatus) { Log.d(TAG, "postNFCStatus(${status})")
        if (NFCManager.isSupportedAndEnabled(getApplication())) {
            ...
            Log.e("NFC Adapter status","Notenabled")
            ...
            Log.e("NFC Adapter status","NotSupported")
        ...
        if (NFCManager.isSupportedAndEnabled(getApplication()) && status == NFCStatus.Tap) {
            Log.e("NFC Adapter status","NFCStatus.Tap")

            liveTag.emit("Tap the NFC Tag")
        } else {
            Log.e("NFC Adapter status","NFCNull")
    ...
    public fun checkRevID(bytes: ByteArray): Boolean {
        var result: Long = 0
        var factor: Long = 1
        for (i in bytes.indices.reversed()) {
            val value = bytes[i].and(0xffL.toByte()).toLong()
        ...
        return result == 429795565L // Check if reversed ID is equal to 12345543
    }
